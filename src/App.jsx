...
function Card({ title, desc, Icon }) {
  return (
    <div className="group rounded-3xl border border-cyan-300/30 bg-white/[0.08] p-6 text-white shadow-card backdrop-blur-lg transition-all duration-300 hover:-translate-y-2 hover:border-cyan-300/70 hover:bg-cyan-300/10 hover:shadow-[0_18px_45px_rgba(34,211,238,.22)]">
      <Icon
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-slate-950 text-white">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-slate-950/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 md:px-5 md:py-5">
          <a href="#home" className="flex min-w-0 items-center gap-3 md:gap-4">
            <img
              src="/logo.png"
              alt="尚順國際運通有限公司"
              className="h-14 w-auto rounded-xl bg-white/95 p-1.5 shadow-xl sm:h-16 md:h-20"
            />
            <div>
              <p className="text-base font-black leading-tight tracking-wide text-white sm:text-lg md:text-2xl">
                尚順國際運通有限公司
              </p>
              <p className="text-xs font-medium text-cyan-300 sm:text-sm md:text-lg">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 py-20 lg:grid-cols-[1.08fr_.92fr]">
          <div>
            ...
            <div className="mt-8 flex flex-col gap-4 pb-[120px] sm:flex-row sm:flex-wrap md:pb-0">
              <Button href="#quote" className="w-full justify-between rounded-[28px] px-7 py-4 text-base sm:w-auto">
                立即詢價 <ArrowRight size={22}/>
              </Button>

              <Button href="#process" variant="secondary" className="w-full justify-between rounded-[28px] px-7 py-4 text-base sm:w-auto">
                查看集運流程 <ArrowRight size={22}/>
              </Button>

              <Button href={company.line} className="w-full justify-between rounded-[28px] px-7 py-4 text-base sm:w-auto">
              ...
              <Button href={`tel:${company.phone}`} variant="secondary" className="w-full justify-between rounded-[28px] px-7 py-4 text-base sm:w-auto">
                <span className="inline-flex items-center gap-2">
          ...
          <div className="rounded-[2rem] border border-cyan-300/25 bg-white/[0.08] p-5 shadow-card backdrop-blur-xl">
            <div className="rounded-[1.5rem] bg-slate-950/30 p-6">
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {['中國貨回台', '淘寶集運回台', '拼多多集運回台', '1688集運回台', '小三通物流', '茶葉小三通', '東南亞物流', '倉儲服務'].map(item => (
                  <div key={item} className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white/5 p-4">
      ...
      <section id="keywords" className="bg-gradient-to-br from-slate-950 via-blue-950 to-indigo-950 py-20 text-white md:py-24"><div className="mx-auto max-w-7xl px-5">
      ...
      <section id="shopping" className="bg-gradient-to-br from-blue-950 via-sky-900 to-cyan-800 py-20 text-white md:py-24">
        <div className="mx-auto max-w-7xl px-5">
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            {shoppingItems.map(([title, desc]) => (
              <div key={title} className="rounded-3xl border border-cyan-300/30 bg-white/[0.08] p-6 text-white shadow-card backdrop-blur-lg">
          ...
          <div className="mt-8 rounded-3xl border border-cyan-300/30 bg-white/[0.08] p-6 text-white backdrop-blur-lg">
      ...
      <section id="services" className="bg-gradient-to-br from-slate-950 via-blue-950 to-indigo-950 py-20 text-white md:py-24"><div className="mx-auto max-w-7xl px-5">
      ...
      <section id="special" className="bg-gradient-to-br from-slate-950 via-indigo-950 to-blue-950 py-20 text-white md:py-24">
        <div className="mx-auto max-w-7xl px-5">
          <SectionTitle dark eyebrow="SPECIAL CARGO" title="特殊貨物也可協助評估" desc="化工品、敏感貨、電池、大型家具與大型機具，請提供完整產品資料及照片，我司協助評估合適方案。" />
          <div className="grid gap-4 md:grid-cols-5">
            {special.map(item => (
              <div key={item} className="rounded-2xl border border-cyan-300/30 bg-white/[0.08] p-5 text-center font-black text-cyan-50 backdrop-blur">
                ✔ {item}
              </div>
            ))}
          </div>
      ...
      <section id="process" className="bg-gradient-to-br from-blue-950 via-sky-900 to-cyan-800 py-20 text-white md:py-24"><div className="mx-auto max-w-7xl px-5">
        <SectionTitle eyebrow="PROCESS" title="物流流程清楚透明" desc="從需求確認、產品資料評估、報價到入倉、清關與派送，降低溝通成本與運輸風險。" />
        <div className="grid gap-4 md:grid-cols-6">
          {steps.map((step, i) => (
            <div key={step} className="rounded-3xl border border-cyan-300/30 bg-white/[0.08] p-5 text-white shadow-card backdrop-blur-lg">
      ...
      <section className="relative bg-gradient-to-br from-blue-950 via-sky-900 to-cyan-800 py-20 text-white md:py-24">
        <div className="mx-auto grid max-w-7xl gap-10 px-5 lg:grid-cols-2">
          <div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {reasons.map(item => <div key={item} className="flex items-center gap-3 rounded-2xl border border-cyan-300/30 bg-white/[0.08] p-4 text-white shadow-card backdrop-blur-lg"><CheckCircle2 className="text-cyan-300" size={20}/>{item}</div>)}
            </div>
          </div>
          <div>
            <SectionTitle eyebrow="ROUTES" title="中國集貨據點與台灣配送" desc="常見中國出貨區域與台灣端配送服務，可依實際貨件狀況安排。" />
            <div className="flex flex-wrap gap-3">
              {routes.map(item => <span key={item} className="rounded-full border border-cyan-300/30 bg-white/[0.08] px-4 py-2 font-bold text-cyan-50 backdrop-blur">{item}</span>)}
            </div>
          </div>
        </div>
      </section>

      <section id="faq" className="bg-gradient-to-br from-slate-950 via-blue-950 to-indigo-950 py-20 text-white md:py-24"><div className="mx-auto max-w-7xl px-5">
        <SectionTitle eyebrow="FAQ" title="客戶常見問題" />
        <div className="grid gap-4 md:grid-cols-2">
          {faqs.map(([q, a]) => (
            <div key={q} className="rounded-3xl border border-cyan-300/30 bg-white/[0.08] p-6 text-white shadow-card backdrop-blur-lg">
      ...
      <section id="quote" className="bg-gradient-to-br from-slate-950 via-blue-900 to-cyan-800 py-20 text-white md:py-24">
        <div className="mx-auto max-w-7xl px-5">
          <SectionTitle dark eyebrow="QUOTE" title="線上詢價" desc="可填寫表單或直接掃描 LINE QR Code，提供產品資料、照片、數量、尺寸重量與出貨需求。" />
          <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
            <div className="overflow-hidden rounded-3xl border border-cyan-300/30 bg-white">
              <iframe src={company.formEmbed} width="100%" height="760" frameBorder="0" marginHeight="0" marginWidth="0">正在載入…</iframe>
            </div>
            <div className="rounded-3xl border border-cyan-300/30 bg-white/[0.08] p-6 text-center shadow-card backdrop-blur-lg">
      ...
      <section id="contact" className="bg-gradient-to-br from-blue-950 via-sky-900 to-cyan-800 py-20 text-white md:py-24"><div className="mx-auto max-w-7xl px-5">
        <SectionTitle eyebrow="CONTACT" title="聯絡尚順國際運通有限公司" />
        <div className="grid gap-8 lg:grid-cols-2">
          <div className="rounded-3xl border border-cyan-300/30 bg-white/[0.08] p-6 text-white shadow-card backdrop-blur-lg">
