...
const services = [
  {
    icon: Ship,
    title: "海運服務",
    desc: "整櫃、併櫃、大貨回台，依貨件重量、材積與時效安排合適方案。",
  },
  {
    icon: Plane,
    title: "空運服務",
    desc: "適合急件、樣品、小批量貨件，協助快速安排航班與後段派送。",
  },
  {
    icon: Globe2,
    title: "小三通物流",
    desc: "熟悉兩岸物流操作，包含一般貨品、茶葉及特殊需求回台安排。",
  },
  {
    icon: FileCheck,
    title: "報關清關",
    desc: "協助確認品名、材質、用途、規格、貨值等資料，降低清關風險。",
  },
  {
    icon: Warehouse,
    title: "倉儲集貨",
    desc: "提供入倉、集貨、核對、貼標、分貨與出貨安排等服務。",
  },
  {
    icon: Truck,
    title: "後段派送",
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        .logo {
          ...
          object-fit: contain; box-shadow: 0 10px 28px rgba(15,23,42,.12);
        }
        .brand-title { font-size: 22px; font-weight: 900; letter-spacing: 1px; }
        .brand-subtitle { font-size: 11px; letter-spacing: 2px; color: #64748b; margin-top: 3px; }
        .nav { display: flex; gap: 28px; color: #475569; font-size: 15px; font-weight: 700; }
        .nav a:hover { color: #0f172a; }
        .header-cta {
          padding: 12px 18px; border-radius: 16px; background: #0f172a;
          color: #fff; font-weight: 800; white-space: nowrap;
        }
        .hero {
          position: relative; overflow: hidden; color: white;
          background:
            linear-gradient(120deg, rgba(3,20,45,.95), rgba(10,62,121,.82)),
        ...
        .hero-inner {
          position: relative; max-width: 1200px; margin: 0 auto; padding: 96px 20px 92px;
        ...
        .tag {
          ...
          background: rgba(255,255,255,.12); color: #dbeafe; font-weight: 800; margin-bottom: 22px;
        }
        .hero h1 { margin: 0; font-size: clamp(42px, 6vw, 72px); line-height: 1.08; letter-spacing: -1.5px; }
        .hero h1 span { display: block; margin-top: 14px; color: #dbeafe; font-size: clamp(28px, 4vw, 46px); }
        .hero p { margin: 26px 0 0; color: #e2e8f0; font-size: 18px; line-height: 2; max-width: 680px; }
        .hero-actions { display: flex; flex-wrap: wrap; gap: 14px; margin-top: 34px; }
        .btn-primary, .btn-secondary {
          display: inline-flex; align-items: center; justify-content: center; gap: 10px;
          padding: 15px 22px; border-radius: 16px; font-weight: 900; transition: .25s ease;
        }
        .btn-primary { background: #2563eb; color: #fff; }
        .btn-secondary { background: rgba(255,255,255,.12); border: 1px solid rgba(255,255,255,.32); color: #fff; }
        .btn-primary:hover, .btn-secondary:hover { transform: translateY(-2px); }
        .hero-card {
          border-radius: 32px; padding: 28px; background: rgba(255,255,255,.13);
          border: 1px solid rgba(255,255,255,.22); box-shadow: 0 35px 90px rgba(0,0,0,.32);
          backdrop-filter: blur(16px);
        }
        .hero-card-title { display: flex; align-items: center; justify-content: space-between; gap: 18px; margin-bottom: 22px; }
        .hero-card-title strong { display: block; font-size: 28px; }
        .hero-card-title span { color: #bfdbfe; font-weight: 700; }
        .route-box {
          height: 210px; border-radius: 24px; position: relative; overflow: hidden;
        ...
        .route-line {
          position: absolute; left: 18%; top: 45%; width: 62%; height: 2px;
          border-top: 2px dashed #93c5fd; transform: rotate(12deg); animation: routeGlow 2.8s infinite;
        }
        .route-line.two { top: 56%; width: 55%; transform: rotate(-10deg); }
        .route-dot {
          position: absolute; width: 44px; height: 44px; border-radius: 50%; background: white; color: #0f172a;
        ...
        .section h2 { margin: 0; font-size: clamp(30px, 4vw, 44px); letter-spacing: -1px; }
        .section-title p { margin: 15px 0 0; color: #64748b; line-height: 1.9; font-size: 17px; }
        .service-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 18px; }
        .service-card {
          background: white; border: 1px solid #e5e7eb; border-radius: 28px; padding: 28px;
          box-shadow: 0 16px 42px rgba(15,23,42,.06); transition: .25s ease;
        }
        .service-card:hover { transform: translateY(-6px); box-shadow: 0 25px 55px rgba(15,23,42,.1); }
        .service-icon {
          width: 52px; height: 52px; border-radius: 18px; background: #eff6ff; color: #2563eb;
          display: grid; place-items: center; margin-bottom: 18px;
        }
        .service-card h3 { margin: 0 0 10px; font-size: 21px; }
        .service-card p { margin: 0; color: #64748b; line-height: 1.8; }
        ...
        .process-item {
          border-radius: 24px; padding: 20px; background: rgba(255,255,255,.09);
          border: 1px solid rgba(255,255,255,.1);
        }
        .process-no {
          width: 38px; height: 38px; border-radius: 50%; background: white; color: #0f172a;
          display: grid; place-items: center; font-weight: 900; margin-bottom: 14px;
        }
        .about { display: grid; grid-template-columns: .95fr 1.05fr; gap: 46px; align-items: center; }
        .about-visual {
          min-height: 380px; border-radius: 34px;
          position: relative;
          overflow: hidden;
          background: #e2e8f0;
        ...
        .about-visual::after {
          ...
          position: absolute;
          inset: 0;
          background: linear-gradient(135deg, rgba(15,23,42,.18), rgba(37,99,235,.28));
        }
        .about-badge {
          position: relative;
          z-index: 2;
          border-radius: 24px; background: rgba(15,23,42,.72); color: white;
          padding: 22px; max-width: 320px; backdrop-filter: blur(10px);
        }
        .about-badge strong { display: block; font-size: 20px; margin-bottom: 6px; }
        .about-text p { color: #64748b; line-height: 2; font-size: 17px; }
        .advantage-list { display: grid; gap: 12px; margin-top: 22px; }
        .advantage-item { display: flex; align-items: flex-start; gap: 12px; font-weight: 800; color: #1e293b; }
        .advantage-item svg { color: #2563eb; flex: none; margin-top: 2px; }
        .contact-section { background: white; }
        .contact-box {
          max-width: 1200px; margin: 0 auto; padding: 52px; border-radius: 34px;
          background: radial-gradient(circle at 85% 12%, rgba(96,165,250,.32), transparent 26%),
            linear-gradient(135deg, #0f172a, #0b3a75);
          color: white; display: grid; grid-template-columns: .95fr 1.05fr; gap: 36px;
          box-shadow: 0 28px 80px rgba(15,23,42,.22);
        }
        .contact-box h2 { margin: 0; font-size: clamp(30px, 4vw, 44px); }
        .contact-box p { color: #dbeafe; line-height: 2; font-size: 17px; }
        .contact-grid { display: grid; gap: 14px; }
        .contact-card, .social-card {
          display: flex; align-items: center; gap: 14px; padding: 18px; border-radius: 22px;
        ...
        .fb { background: #1877f2; border: none; }
        .line { background: #06c755; border: none; }
        .map { background: white; color: #0f172a; border: none; }
        .map svg { color: #2563eb; }
        ...
        .form-title h2 { margin: 0; font-size: clamp(30px, 4vw, 44px); }
        .form-title p { color: #64748b; line-height: 1.8; }
        .form-frame {
          overflow: hidden; border-radius: 32px; border: 1px solid #e5e7eb; background: white;
          box-shadow: 0 22px 65px rgba(15,23,42,.09);
        }
        .form-frame iframe { display: block; width: 100%; min-height: 2546px; border: 0; }
        ...
        @media (max-width: 980px) {
          .nav { display: none; }
          .hero-inner, .about, .contact-box { grid-template-columns: 1fr; }
        ...
        @media (max-width: 640px) {
          ...
          .form-frame iframe { min-height: 2600px; }
        }
      `}</style>
      ...
      <header className="header">
        <div className="header-inner">
          <a className="brand" href="#top">
            ...
            <div>
              <div className="brand-title">尚順國際運通</div>
      ...
      <main id="top">
            ...
            <div>
              ...
              <div className="hero-actions">
                ...
                <a className="btn-secondary" href={companyInfo.line} target="_blank" rel="noreferrer">
                  LINE 即時詢價 <MessageCircle size={18} />
                </a>
              </div>
            </div>
        ...
        <section id="about" className="section about">
          <div className="about-visual">
            <img src={companyInfo.aboutImage} alt="國際物流貨櫃運輸" />
          ...
          <div className="about-text">
            ...
            <h2>重視每一票貨件的細節與安全</h2>
            <p>
              物流不只是把貨送到目的地，更重要的是事前資料確認、風險控管與過程追蹤。
              尚順國際運通重視每一票貨件的操作細節，協助客戶降低清關延誤、
              貨損與溝通成本，讓貨件運輸更穩定、更安心。
            </p>
          ...
          <div className="section">
            <div className="contact-box">
              ...
              <div className="contact-grid">
                ...
                <a className="contact-card" href={companyInfo.googleMap} target="_blank" rel="noreferrer">
                  <MapPin size={24} />
                  <div><span>公司地址</span><strong>{companyInfo.address}</strong></div>
                </a>
                ...
                <a className="social-card line" href={companyInfo.line} target="_blank" rel="noreferrer">
                  <MessageCircle size={22} /> LINE 即時詢價
                </a>
